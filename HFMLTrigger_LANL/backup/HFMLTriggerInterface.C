...
//	vector<vector<int>> HitInfo_INTT;
	...
	//	cout << "INTT: trkid =  " << trkid << "  hitIDINTT =  " << hitIDINTT << endl;
	  ...
	  if(layer > _nlayers_maps-1){
				...
				unsigned int LadderZId = InttDefs::getLadderZId(hitKey);
				unsigned int LadderPhiId = InttDefs::getLadderPhiId(hitKey);
...
//	cout << "ZZ Check: INTTHITS = " << INTTHITS << endl;
			...
			++pp_iter)
	{
		...
		if (! m_track_g4hits_map_intt.count(g4particle->get_track_id()))
		{
			if (Verbosity() >= VERBOSITY_MORE)
				std::cout << "WARNING: G4 particle " << g4particle->get_track_id() \
					<< " does not hit any INTT layer." << std::endl;
			continue;
		}
...
//		cout << "Pass 6.5" << endl;
		...
		PHG4Hit *g4hit = m_g4hits_mvtx->findHit( g4hits_iter.first->second );
		int trkid = g4hit->get_trkid();
		
		//cout << "g4particle->get_track_id() = " << g4particle->get_track_id() << "    trkid = " << trkid << endl;


		rapidjson::Value MVTXHitIDTree2(rapidjson::kArrayType);
		
		int TrkIDVecSize = TrkIDVec.size();

		for(int i = 0; i < TrkIDVecSize; i++){

		
			if(trkid == TrkIDVec[i]) IndexVec.push_back(i);
...
//		cout << "Pass 6.7" << endl;
		...
		PHG4Hit *g4hit_intt = m_g4hits_intt->findHit( g4hits_iter_intt.first->second );
	
	

		int trkid_intt = g4hit_intt->get_trkid();

	


		//cout << "trkid_intt = " << trkid_intt << "   trkid = " << trkid << endl;
	...
	//	cout << "Pass 6.10" << endl;
		...
		int TrkIDVecSize_intt = TrkIDVecINTT.size();
	


	//	cout << "Pass 6.11" << endl;

		for(int i = 0; i < TrkIDVecSize_intt; i++){

		
			if(trkid_intt == TrkIDVecINTT[i]) IndexVecINTT.push_back(i);
