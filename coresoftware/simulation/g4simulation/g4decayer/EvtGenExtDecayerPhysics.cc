...
void EvtGenExtDecayerPhysics::ConstructProcess()
{
  ...
  std::cout << 	"Using My Fuckin Decayer" << std::endl;
  std::cout << 	"Is Input File Empty???? -  Answer:   " <<    DecayFile.empty() << std::endl;
  ...
  if (!DecayFile.empty())
  {
    ...
    if (f_local.good())
    {
	  std::cout << 	"DecayFile?" << DecayFile << std::endl;
      extDecayer->SetDecayTable(DecayFile, false);
    }
    else
    {
	  std::cout << 	"OK - No Decay" << std::endl;
      ...
      if (f_calib.good())
      {
 	    std::cout << "OK I set = " << DecayFileCalibrationRepo << std::endl;  
	  ...
	  std::cout << "DecayFileCalibrationRepo = " << DecayFileCalibrationRepo << std::endl;
    }
  }
